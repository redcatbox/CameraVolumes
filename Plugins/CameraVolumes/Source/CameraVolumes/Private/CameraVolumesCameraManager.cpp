...
void ACameraVolumesCameraManager::CalcNewCameraParams(ACameraVolumeActor* CameraVolume, float DeltaTime)
{
	if (CameraVolume)
	{
		...
		if (CameraVolume->GetIsCameraStatic())
		{
			...
			{
				FVector FocalPoint = CameraVolume->CameraFocalPoint + PlayerPawnLocationTransformed;
				NewCameraRotation = UCameraVolumesFunctionLibrary::CalculateCameraRotation(CameraVolume->CameraLocation, FocalPoint, CameraVolume->CameraRoll);
			}
			else
		...
		else
		{
			...
			if (bBlockingCalculations)
			{
				...
				FVector NewCamVolExtentCorrected = CameraVolume->VolumeExtent;
				...
				FVector ScreenMin = -NewCamVolExtentCorrected + ScreenExtent;
				FVector ScreenMax = NewCamVolExtentCorrected - ScreenExtent;
